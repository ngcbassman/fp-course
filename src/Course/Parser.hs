...
character ::
  Parser Char
character = 
  P $ \input -> if isEmpty input then UnexpectedEof else 
    let (h :. t) = input in
    Result t h 
...
(|||) ::
  ...
  -> Parser a
(|||) (P a1) (P a2) = P $ \input -> 
  let result = a1 input in
  if (isErrorResult result) then a2 input else result
...
satisfy ::
  (Char -> Bool)
  -> Parser Char
satisfy f = bindParser (\a -> if f a then valueParser a else unexpectedCharParser a) character
...
sequenceParser ::
  List (Parser a)
  -> Parser (List a)
sequenceParser = foldRight (\pa acc -> bindParser (\a -> (:.) <$> valueParser a <*> acc) pa) (valueParser Nil)
...
thisMany ::
  ...
  -> Parser a
  -> Parser (List a)
thisMany quantity pa = sequenceParser $ replicate quantity pa
...
firstNameParser ::
  Parser Chars
firstNameParser = bindParser (\a -> (:.) <$> valueParser a <*> list lower) upper
